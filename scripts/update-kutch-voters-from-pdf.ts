/**
 * Update Kutch zone voter assignments based on updated PDF
 */
...
async function updateKutchVotersFromPDF() {
  try {
    console.log('\n' + '='.repeat(80))
    console.log('UPDATING KUTCH ZONE VOTERS FROM UPDATED PDF')
    ...
    // Create sets of PDF voter identifiers
    const pdfVoterIds = new Set<string>()
    const pdfPhones = new Set<string>()
    
    pdfVoters.forEach(voter => {
      if (voter.voterId) {
        pdfVoterIds.add(voter.voterId.toUpperCase())
      }
      pdfPhones.add(voter.phone)
    })
    
    // Step 1: Assign all PDF voters to Kutch zone
    let assigned = 0
    ...
    // Step 2: Remove voters from Kutch zone that are not in PDF
    const currentKutchVoters = await prisma.voter.findMany({
      where: {
        yuvaPankZoneId: kutchZone.id,
        voterId: {
          not: {
    ...
    for (const dbVoter of currentKutchVoters) {
      const dbPhone = normalizePhone(dbVoter.phone) || ''
      const dbVoterId = dbVoter.voterId.toUpperCase()
      
      const inPDF = pdfVoterIds.has(dbVoterId) || pdfPhones.has(dbPhone)
      
      if (!inPDF) {
        // Remove from Kutch zone (set to null)
        await prisma.voter.update({
          where: { id: dbVoter.id },
          data: {
            yuvaPankZoneId: null
        ...
        removed++
        console.log(`   ➖ Removed from Kutch: ${dbVoter.name} (${dbVoter.voterId})`)
      }
    }
    ...
    console.log(`➖ Removed from Kutch: ${removed}`)
    ...
    console.log(`📊 Expected count from PDF: ${pdfVoters.length}`)
    console.log(`📊 Match: ${finalCount === pdfVoters.length ? '✅ YES' : '❌ NO'}`)
    console.log('='.repeat(80))
